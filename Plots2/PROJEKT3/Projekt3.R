...
#air_data <- air_data[air_data$Year == "2019" | air_data$Year == "2018",]

#wars1 <- air_data[air_data$Loc == 'Warszawa' & air_data$Hour=="01:00",]
for (i in 1:24)  {      
  x <- air_data[air_data$Loc == 'Warszawa' & air_data$Hour_numb==i,]
  assign(paste("Warszawa", i, sep = "_") , x)      
}

for (i in 1:24)  {      
  x <- air_data[air_data$Loc == 'Wrocław' & air_data$Hour_numb==i,]
  assign(paste("Wrocław", i, sep = "_") , x)      
}

for (i in 1:24)  {      
  x <- air_data[air_data$Loc == 'Gdańsk' & air_data$Hour_numb==i,]
  assign(paste("Gdańsk", i, sep = "_") , x)      
}

for (i in 1:24)  {      
  x <- air_data[air_data$Loc == 'Kraków' & air_data$Hour_numb==i,]
  assign(paste("Kraków", i, sep = "_") , x)      
}

for (i in 1:24)  {      
  x <- air_data[air_data$Loc == 'Łódź' & air_data$Hour_numb==i,]
  assign(paste("Łódź", i, sep = "_") , x)      
}

for (i in 1:24)  {      
  x <- air_data[air_data$Loc == 'Nowa Ruda' & air_data$Hour_numb==i,]
...
x <- 1:24
png(file="godziny_all.png", width = 1600, height = 800)
plot(x, df$Warszawa, ylim=c(0,85), cex=2, lwd=2, col="#CE7D3B", pch=0, cex.lab = 1.5, cex.main = 2.2, cex.axis = 1.5,
     type='b', xlab="Godzina", ylab="Stężenie [µg/m³]", main="Średnie stężenie godzinowe pyłu PM10", xaxt = "n",)
points(x, df$Wrocław, pch = 19, type='b', col="#6F9F00")
points(x, df$Gdańsk, pch=2, type='b', lwd=2,cex=2,  col="#00AD86")
points(x, df$Kraków, pch=8, type='b', lwd=2,cex=2,   col="#009ADE")
points(x, df$Łódź, pch=3, type='b', lwd=1, col="#9F7FE5")
points(x, df$Nowa_Ruda, pch=18, lwd=2,cex=2,  type='b', col="#E3669B")
legend(0.09, 87, legend=c("Warszawa", "Wrocław", "Gdańsk", "Kraków", "Łódź", "Nowa Ruda"),
       col=c("#CE7D3B", "#6F9F00", "#00AD86", "#009ADE", "#9F7FE5", "#E3669B"), lty=c(1,1,1,1,1,1), pch=c(0,19,2,8,3,18), cex=1.4)
