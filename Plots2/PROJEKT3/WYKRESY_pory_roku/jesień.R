...
for (i in 1:24)  {      
  x <- air_data[air_data$Loc == 'Warszawa' & air_data$Hour_numb==i & (air_data$Month == 9 | air_data$Month == 10 | air_data$Month == 11),]
  assign(paste("Warszawa", i, sep = "_") , x)      
}

for (i in 1:24)  {      
  x <- air_data[air_data$Loc == 'Wrocław' & air_data$Hour_numb==i & (air_data$Month == 9 | air_data$Month == 10 | air_data$Month == 11),]
  assign(paste("Wrocław", i, sep = "_") , x)      
}

for (i in 1:24)  {      
  x <- air_data[air_data$Loc == 'Gdańsk' & air_data$Hour_numb==i & (air_data$Month == 9 | air_data$Month == 10 | air_data$Month == 11),]
  assign(paste("Gdańsk", i, sep = "_") , x)      
}

for (i in 1:24)  {      
  x <- air_data[air_data$Loc == 'Kraków' & air_data$Hour_numb==i & (air_data$Month == 9 | air_data$Month == 10 | air_data$Month == 11),]
  assign(paste("Kraków", i, sep = "_") , x)      
}

for (i in 1:24)  {      
  x <- air_data[air_data$Loc == 'Łódź' & air_data$Hour_numb==i & (air_data$Month == 9 | air_data$Month == 10 | air_data$Month == 11),]
  assign(paste("Łódź", i, sep = "_") , x)      
}

for (i in 1:24)  {      
  x <- air_data[air_data$Loc == 'Nowa Ruda' & air_data$Hour_numb==i & (air_data$Month == 9 | air_data$Month == 10 | air_data$Month == 11),]
...
x <- 1:24
png(file="jesien.png", width = 1000, height = 500)
plot(x, df$Warszawa, ylim=c(0,120), col="#CE7D3B", pch=0, cex.lab = 1.5, cex.main = 2.2, cex.axis = 1.5,
     type='b', xlab="Godzina", ylab="Stężenie [µg/m³]", main="Średnie stężenie godzinowe pyłu PM10 - jesień", xaxt = "n",)
