...
wars_val <- df[df$Loc == 'Warszawa' & df$Type == "PM10" & (df$Year=="2019" | df$Year=="2018" | 
                                                            df$Year=="2017" | df$Year=="2016" | df$Year=="2015"),]
wars_data1 <- wars_val %>% select(Value)

wars_weather1 <- df_w[df_w$Loc == 'Warszawa' & (df_w$Year=="2019" | df_w$Year=="2018" | 
...
wind_value_plot <- ggplot(wind_arr,  mapping=aes(x=Wind, y=Value)) +
  geom_point(aes(col='Wartość'), size=3) +
  ggtitle('Warszawa') +
...
png(file="Warszawa_wiatr.png", width = 1400, height = 800)
wind_value_plot 
dev.off()

png(file="Warszawa_temp.png", width = 1400, height = 800)
temp_value_plot 
dev.off()

png(file="Warszawa_wilg.png", width = 1400, height = 800)
hum_value_plot 
dev.off()
